...
将来的には、運用中の **webw** と連携した着せ替え機能のために、Fledge 独自アカウントや利用率解析を導入する可能性があります。その場合でも、個人情報・認証情報・秘密情報をソースコードや公開リポジトリへ含めないことを前提にします。実装時は `PRIVACY.md` と README を更新してください。
