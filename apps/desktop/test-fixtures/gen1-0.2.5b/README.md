# 第1世代ファイナル（Ver.0.2.5b）テスト用仮データ

0.2.4b 相当の **exe 横データレイアウト** のまま、**更新上限パッチ**（0.3 以上を案内しない）を入れた状態を再現するためのフィクスチャです。  
本番の `Ver.0.2.5b` リリースは `v0.2.4b` から上限パッチのみ cherry-pick して切ります（main の 0.3.x とは別系統）。
...
updater-cache/
  ...
  with-ceiling-up-to-date-on-0.2.5b.json … 上限あり・0.2.5b 時点で最新扱い
...
|--------|---------------|--------------|
| 0.2.4b | 0.3.0a (prerelease) | **0.2.5b** または 0.2.x 系最新（0.3 は無視） |
| 0.2.5b | 0.3.0a | **更新なし**（第1世代内最新） |
