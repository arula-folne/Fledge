...
export async function spawnInstallerAfterAppExit(
  ...
  const content = [
    ...
    // ロック解除の最短待ち（timeout はコンソール無しで危険なので ping）
    'ping -n 2 127.0.0.1 >nul',
    ...
    '  exit /b !EC!',
    ')',
    // スタブ経由だと二重起動になるので runtime を直接起動（速い）
    'set "RUNTIME=!INSTALLDIR!\\data\\meta\\runtime\\Fledge.exe"',
    'if exist "!RUNTIME!" (',
    '  echo starting runtime>>"!LOG!"',
    '  start "" /D "!INSTALLDIR!\\data\\meta\\runtime" "!RUNTIME!" --updated',
    ') else if exist "!APP!" (',
    '  echo starting stub>>"!LOG!"',
    '  start "" "!APP!" --updated',
    ') else (',
