...
export function reconcileCachedUpdateResult(
  ...
  if (
    cached.nextVersion &&
    ((isGeneration1App(currentVersion) && !isEligibleGeneration1Update(cached.nextVersion)) ||
      (isGeneration2App(currentVersion) && !isEligibleGeneration2Update(cached.nextVersion)))
  ) {
    return null
  }
...
export class GithubReleaseUpdater implements Updater {
  ...
  private async fetchRelease(channel: UpdateChannel): Promise<GithubRelease> {
    ...
    const gen2 = isGeneration2App(currentVersion)

    // 世代ロックがあるビルドは /releases/latest を使わない（次世代が Latest になると飛び越える）
    if (!gen1 && !gen2 && channel === 'stable') {
      return this.fetchLatestStableRelease()
    }

    return this.fetchNewestFromList(channel, gen1, gen2)
  }
  ...
  private async fetchNewestFromList(
    ...
    const timer = setTimeout(() => controller.abort(), UPDATER.fetchTimeoutMs)
    const perPage =
      gen1 || gen2 ? UPDATER.generationReleaseListPerPage : 20
    ...
    try {
      ...
      if (gen1) {
        ...
        releases = releases.filter((release) =>
          isEligibleGeneration2Update(normalizeReleaseVersion(release.tag_name)),
        )
      }
