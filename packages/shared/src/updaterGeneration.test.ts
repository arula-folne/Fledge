...
test('compareVersions treats 0.2.6b as newer than earlier gen1 finals', () => {
  assert.equal(compareVersions('0.2.4b', '0.2.6b'), -1)
  assert.equal(compareVersions('0.2.4f', '0.2.6b'), -1)
  assert.equal(compareVersions('0.2.5b', '0.2.6b'), -1)
  assert.equal(compareVersions('0.2.6b', GEN1_FINAL_VERSION), 0)
})
...
test('generation-1 apps exclude 0.3+ updates', () => {
  ...
  assert.equal(isEligibleGeneration1Update('0.2.6b'), true)
